...
func UpdateUser(ctx *context.Context, resp http.ResponseWriter, req *http.Request) {
	...
	// Double check authorization
	if user == nil {
		ctx.Forbidden("you need to be authenticated, please login first")
		return
	}
	...
	err = json.Unmarshal(body, userParams)
	if err != nil {
		ctx.BadRequest("unable to deserialize to user : %s", err)
		return
	}

	if userParams.ID != user.ID {
		ctx.BadRequest("user id missmatch")
		return
	}
	...
	err = json.Unmarshal(body, password)
	if err != nil {
		ctx.BadRequest("unable to deserialize to password : %s", err)
		return
	}
	userParams.Password = password.Password
